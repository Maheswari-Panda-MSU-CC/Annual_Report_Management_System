...
export async function POST(request: NextRequest) {
  try {
    ...
    if (visit.doc && typeof visit.doc === 'string' && visit.doc.startsWith('upload/')) {
      // Extract entity name from virtual path
      let entityName = 'S3_Document';
      if (visit.doc.startsWith('upload/')) {
        const pathWithoutUpload = visit.doc.substring(7); // Remove 'upload/'
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (visit.doc && typeof visit.doc === 'string' && visit.doc.startsWith('upload/')) {
      ...
      // Log S3_UPLOAD activity with the correct recordId (non-blocking)
      logActivityFromRequest(request, user, 'S3_UPLOAD', entityName, visitId).catch(() => {})
    }
    ...
    if (docPath && typeof docPath === 'string' && docPath.trim() && docPath.startsWith('upload/')) {
      try {
        ...
        if (s3DeleteResult.success) {
          ...
          logActivityFromRequest(request, user, 'S3_DELETE', entityName, visitId).catch(() => {})
        } else {
