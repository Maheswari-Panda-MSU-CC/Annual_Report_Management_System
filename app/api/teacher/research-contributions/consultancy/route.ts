...
export async function POST(request: NextRequest) {
  try {
    ...
    if (consultancy.doc && typeof consultancy.doc === 'string' && consultancy.doc.startsWith('upload/')) {
      // Extract entity name from virtual path
      let entityName = 'S3_Document';
      if (consultancy.doc.startsWith('upload/')) {
        const pathWithoutUpload = consultancy.doc.substring(7); // Remove 'upload/'
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (consultancy.doc && typeof consultancy.doc === 'string' && consultancy.doc.startsWith('upload/')) {
      ...
      // Log S3_UPLOAD activity with the correct recordId (non-blocking)
      logActivityFromRequest(request, user, 'S3_UPLOAD', entityName, consultancyId).catch(() => {})
    }
    ...
    if (docPath && typeof docPath === 'string' && docPath.trim() && docPath.startsWith('upload/')) {
      try {
        ...
        if (s3DeleteResult.success) {
          ...
          logActivityFromRequest(request, user, 'S3_DELETE', entityName, consultancyId).catch(() => {})
        } else {
