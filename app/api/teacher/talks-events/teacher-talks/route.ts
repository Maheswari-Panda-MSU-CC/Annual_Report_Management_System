...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await req.execute('sp_InsertTeacherTalk')
    const insertedId = result.recordset?.[0]?.id || result.returnValue || null

    // Log activity (non-blocking)
    logActivityFromRequest(request, user, 'CREATE', 'TeacherTalk', insertedId).catch(() => {})
    
    // Log S3_UPLOAD activity if document was uploaded (with correct recordId)
    const docPath = teacherTalk.Image || teacherTalk.doc || teacherTalk.supporting_doc || null
...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Log activity (non-blocking)
    logActivityFromRequest(request, user, 'UPDATE', 'TeacherTalk', teacherTalkId).catch(() => {})
    ...
    if (docPath && typeof docPath === 'string' && docPath.startsWith('upload/')) {
      ...
      // Log S3_UPLOAD activity with the correct recordId (non-blocking)
      logActivityFromRequest(request, user, 'S3_UPLOAD', entityName, teacherTalkId).catch(() => {})
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (docPath && typeof docPath === 'string' && docPath.trim() && docPath.startsWith('upload/')) {
      try {
        ...
        if (s3DeleteResult.success) {
          ...
          logActivityFromRequest(request, user, 'S3_DELETE', entityName, teacherTalkId).catch(() => {})
        } else {
    ...
    // Log activity (non-blocking)
    logActivityFromRequest(request, user, 'DELETE', 'TeacherTalk', teacherTalkId).catch(() => {})
