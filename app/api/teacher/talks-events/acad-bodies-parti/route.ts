...
export async function POST(request: NextRequest) {
  try {
    ...
    const docPath = partiAcads.supporting_doc || partiAcads.doc || partiAcads.Image || null
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (docPath && typeof docPath === 'string' && docPath.startsWith('upload/')) {
      ...
      // Log S3_UPLOAD activity with the correct recordId (non-blocking)
      logActivityFromRequest(request, user, 'S3_UPLOAD', entityName, partiAcadsId).catch(() => {})
    }
    ...
    if (docPath && typeof docPath === 'string' && docPath.trim() && docPath.startsWith('upload/')) {
      try {
        ...
        if (s3DeleteResult.success) {
          ...
          logActivityFromRequest(request, user, 'S3_DELETE', entityName, partiAcadsId).catch(() => {})
        } else {
