...
export async function POST(request: NextRequest) {
  try {
    ...
    const docPath = refresherDetail.supporting_doc || refresherDetail.doc || refresherDetail.Image || null
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (docPath && typeof docPath === 'string' && docPath.startsWith('upload/')) {
      ...
      // Log S3_UPLOAD activity with the correct recordId (non-blocking)
      logActivityFromRequest(request, user, 'S3_UPLOAD', entityName, refresherDetailId).catch(() => {})
    }
    ...
    if (docPath && typeof docPath === 'string' && docPath.trim() && docPath.startsWith('upload/')) {
      try {
        ...
        if (s3DeleteResult.success) {
          ...
          logActivityFromRequest(request, user, 'S3_DELETE', entityName, refresherDetailId).catch(() => {})
        } else {
