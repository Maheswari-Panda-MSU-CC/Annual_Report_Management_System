...
export default function EditResearchPage() {
  ...
  useEffect(() => {
    ...
    reset(formData)
    const pdfUrl = project.Pdf || null
    setDocumentUrl(pdfUrl)
    setExistingDocumentUrl(pdfUrl)
    // Track original document URL to detect changes
    originalDocumentUrl.current = pdfUrl
  ...
  const onSubmit = async (data: ResearchProjectFormData) => {
    ...
    const originalPdfPath = originalDocumentUrl.current // Original document URL from when page loaded
    ...
    const isDocumentChanged = documentUrl !== originalPdfPath
    ...
    if (isNewUpload && isDocumentChanged) {
      ...
      if (isDocumentChanged) {
        ...
        pdfPath = originalPdfPath || existingDocumentUrl || ""
      }
    } else if (!documentUrl && originalPdfPath) {
