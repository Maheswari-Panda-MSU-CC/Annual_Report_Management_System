...
interface ActivitiesTabProps {
  ...
  onDelete: (activityId: number, itemName: string, item?: any) => void
}
...
export function ActivitiesTab({
  ...
  const columns = useMemo<ColumnDef<any>[]>(() => [
    ...
    { 
      ...
      enableSorting: true, 
      cell: ({ row }) => {
        const activity = displayValue(row.original.activity)
        return <div className="font-medium text-xs sm:text-sm px-2 sm:px-4 max-w-[120px] sm:max-w-none truncate" title={activity}>{activity}</div>
      }, 
    ...
    { 
      accessorKey: "participatants_num", 
      header: "No. of Students", 
      enableSorting: true, 
      cell: ({ row }) => <span className="text-xs sm:text-sm px-2 sm:px-4">{displayValue(row.original.participatants_num)}</span>, 
      meta: { className: "text-xs sm:text-sm px-2 sm:px-4" } 
    },
    { 
      id: "participants", 
      accessorFn: (row) => row.participatants_num,
      header: "Participants", 
      enableSorting: true, 
      cell: ({ row }) => <span className="text-xs sm:text-sm px-2 sm:px-4">{displayValue(row.original.participatants_num)}</span>, 
      meta: { className: "text-xs sm:text-sm px-2 sm:px-4" } 
        ...
        return (
          <div className="flex items-center gap-1 sm:gap-2 px-2 sm:px-4" onClick={(e) => e.stopPropagation()}>
            ...
            <Button 
              ...
              onClick={(e) => { 
                e.stopPropagation()
                onDelete(item.id, item.activity || "this record", item)
              }}
    ...
    <Card>
      ...
      <CardContent>
        <EnhancedDataTable
          ...
          emptyMessage="No activities found. Click 'Add Activity' to get started."
